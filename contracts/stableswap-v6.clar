...
(define-constant CONTRACT_ADDRESS 'ST38GBVK5HEJ0MBH4CRJ9HQEW86HX0H9AP3EJP4TW.stableswap-v6)
(define-constant FEE_TO_ADDRESS 'ST38GBVK5HEJ0MBH4CRJ9HQEW86HX0H9AP3EJP4TW.fee-escrow-v2)
...
;; ;; (define-private (sum-up-rewards (rewards-and-principal (list 1000 (optional (list 4 uint)))) (rap-info {x-rewards: uint, y-rewards: uint, lp-principal: uint, xbtc-principal: uint, index: uint}))
;; (define-private (sum-up-rewards (reward-cycle uint) (rap-info {x-rewards: uint, y-rewards: uint, lp-principal: uint, xbtc-principal: uint, index: uint, idk: (list 10 (optional (list 4 uint)))}))
;;   ;;rap-info = rewards and principal info
;;   (let (
;;     (i (get index rap-info))
;;     (idk (get idk rap-info))
;;     (list_i (unwrap! (unwrap! (element-at idk i) PANIC_ERR) PANIC_ERR))
;;     (more-x (unwrap! (element-at list_i u0) PANIC_ERR))
;;     (more-y (unwrap! (element-at list_i u1) PANIC_ERR))
;;     (more-lp (unwrap! (element-at list_i u2) PANIC_ERR))
;;     (more-xbtc (unwrap! (element-at list_i u3) PANIC_ERR))

;;     ;; (this-list (unwrap! (unwrap! (element-at rewards-and-principal i) PANIC_ERR) PANIC_ERR))
;;     ;; (this-list (unwrap! rewards-and-principal PANIC_ERR))
;;     ;; (more-x (unwrap! (element-at this-list u0) PANIC_ERR))
;;     ;; (more-y (unwrap! (element-at this-list u1) PANIC_ERR))
;;     ;; (more-lp (unwrap! (element-at this-list u2) PANIC_ERR))
;;     ;; (more-xbtc (unwrap! (element-at this-list u3) PANIC_ERR))


;;     ;; (more-x (unwrap! (element-at rewards-and-principal u0) PANIC_ERR))
;;     ;; (more-y (unwrap! (element-at rewards-and-principal u1) PANIC_ERR))
;;     ;; (more-lp (unwrap! (element-at rewards-and-principal u2) PANIC_ERR))
;;     ;; (more-xbtc (unwrap! (element-at rewards-and-principal u3) PANIC_ERR))


;;     (x-rewards (get x-rewards rap-info))
;;     (y-rewards (get y-rewards rap-info))
;;     (lp-principal (get lp-principal rap-info))
;;     (xbtc-principal (get xbtc-principal rap-info))


;;     ;; (rap-info-updated 
;;     ;;   (merge rap-info 
;;     ;;     { x-rewards: (+ x-rewards more-x),
;;     ;;       y-rewards: (+ y-rewards more-y),
;;     ;;       lp-principal: (+ lp-principal more-lp),
;;     ;;       xbtc-principal: (+ xbtc-principal more-xbtc),
;;     ;;       index: (+ i u1)}
;;     ;;   ) 
;;     ;; )
;;     (rap-info-updated 
